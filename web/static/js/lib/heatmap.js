...
(function(w){
    ...
    legend.prototype = {
        init: function(){
            ...
            if(position.indexOf('t') > -1){
                positionCss += 'top:'+offsetY+'px;';
            }else{
                positionCss += 'bottom:'+offsetY+'px;';
            }

            // left or right
            if(position.indexOf('l') > -1){
                positionCss += 'left:'+offsetX+'px;';
            }else{
                positionCss += 'right:'+offsetX+'px;';
            }
            element = document.createElement("div"),
            element.style.cssText = "border-radius:3px;position:fixed;"+positionCss+"font-family:Helvetica; width:"+width+"px;z-index:10000000000; background:rgba(255,255,255,1);padding:5px;border:1px solid black;margin:0;";
            element.innerHTML = "<div style='padding:0;margin:0;text-align:center;font-size:12px;'>"+title+"</div>";
            // create gradient in canvas
            labelsEl.style.cssText = "position:relative;font-size:12px;display:block;list-style:none;list-style-type:none;margin:0;height:"+(height)+"px;";
            

            // create gradient element
            gradient = document.createElement("div");
            gradient.style.cssText = ["position:relative;display:block;width:"+width+"px;height:"+(height)+"px;border-bottom:1px solid black; background-image:url(",me.createGradientImage(),");"].join("");
        ...
        createGradientImage: function(){
            ...
            console.log(width)
            grad = ctx.createLinearGradient(0,5,width,height-3);
            ...
            ctx.fillStyle = grad;
            ctx.fillRect(0,5,width,height-3);
            ctx.strokeStyle = "black";
            ctx.beginPath();
 
            for(var i = 0; i < length; i++){
                ctx.moveTo(((1/(length-1)*i*width) >> 0)+.5, 0);
                ctx.lineTo(((1/(length-1)*i*width) >> 0)+.5, (i==0)?15:5);
            }
            ctx.moveTo(width-.5, 0);
            ctx.lineTo(width-.5, 15);
            ctx.moveTo(width-.5, 4.5);
            ctx.lineTo(0, 4.5);
    ...
    heatmap.prototype = {
        configure: function(config){
                ...
                me.set("legendParent", config.legendParent);
        ...
        init: function(){
                ...
                if(me.get("legend")){
                    legendParent = me.get("legendParent")
                    if(legendParent){
                        legendParent.appendChild(me.get("legend").getElement());
                    }else{
